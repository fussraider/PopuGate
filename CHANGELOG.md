...
- **Configurable unknown-SNI action**: per-instance `unknown_sni_action` (`drop`/`mask`/`accept`/`reject_handshake`, default `mask` = prior behavior) for FakeTLS instances. `reject_handshake` (engine ≥ 3.4.4) emits a fatal `unrecognized_name` TLS alert like nginx `ssl_reject_handshake`. Changing it restarts the instance.
- **Client MSS shaping (`client_mss` / `client_mss_bulk`)**: per-instance client-facing TCP MSS (88–4096; 88=extreme-low, 92=tspu, 256=2in8) to fragment the TLS ServerHello and defeat segment-boundary DPI (engine ≥ 3.4.15), plus `client_mss_bulk` to raise the MSS after the handshake and cut packets-per-second (engine ≥ 3.4.19). Linux-only, off by default, restart-required. (Reuses and completes the previously inert `tcp_mss` instance field.)
- **Dual-stack upstream policy**: per-upstream `ipv4`/`ipv6` allow-lists (tri-state: Auto/Allow/Disallow), preferred DC family (`prefer` 4/6), and `bindtodevice` (SO_BINDTODEVICE, direct upstreams only, Linux) — engine 3.4.0/3.4.14. Defaults (Auto) preserve current behavior; restart-required.
- **Class-labeled error metrics**: the live-metrics scraper now parses the engine's `telemt_connections_bad_by_class_total{class}` and `telemt_handshake_failures_by_class_total{class}` (engine ≥ 3.4.11) and shows an "Error classes" diagnostics table in the Traffic view (hidden until data exists) — visibility into *why* connections fail (unknown SNI, PROXY-protocol, timeouts, ...).
- **Per-SNI mask targets (`exclusive_mask`)**: optional per-instance JSON map of SNI → `host:port`, rendered as `[censorship.exclusive_mask]` (engine ≥ 3.4.6). Fallback (non-MTProto) traffic whose SNI matches an entry is proxied to that specific target instead of the single default mask host, so one instance can plausibly front several distinct sites. Empty by default (unchanged behavior); restart-required. Validated as `host:port` with port 1–65535.
- **Immediate quota reset (no restart)**: quota resets now propagate to the running engine via its
  control-plane API (`POST /v1/users/{label}/reset-quota`, engine ≥ 3.4.3) instead of waiting for a
  container recreate to clear the persisted `used_bytes` counter. Applies to the per-secret reset, the
  reset-all action, and the monthly `quota-reset` task — a mid-cycle reset now unblocks a quota-capped
  user right away. Each instance gets an auto-assigned loopback control-plane API listener
  (`[server.api] listen = 127.0.0.1:<api_port>`, new `instances.api_port`), and PopuGate calls it on
  `127.0.0.1`. Best-effort: a failed call is logged and never blocks the SQLite-ledger reset or restarts
  anything. Pre-existing instances are backfilled a port on startup (active after the next recreate).
  Security hardening: the engine otherwise enables this API on `0.0.0.0:9091` by default — we now render
  `[server.api]` explicitly (loopback bind, or `enabled=false` when no port), closing that exposure.
- **SYN flood limiter (opt-in)**: global setting to enable the engine's netfilter-based per-source SYN rate limiter, rendered per bind address as `[[server.listeners]]` `synlimit` (engine ≥ 3.4.17), with configurable backend (`nftables`/`iptables`), window seconds, hitcount, and burst (defaults 60/48/1). Off by default. When enabled, the proxy containers are recreated with `CAP_NET_ADMIN` (required for the engine to install host netfilter rules under `network=host`) and the engine image now always ships `nftables`+`iptables`; disabling recreates them again without the capability. Because a capability change only takes effect at container create, toggling this setting recreates containers (brief interruption) rather than a SIGHUP hot-reload. The `CAP_NET_ADMIN` grant is surfaced with a warning in the Settings UI. (Verified end-to-end on telemt 3.4.22: the engine installs `table inet telemt_synlimit` rate-limit rules for both IPv4 and IPv6 on the listener port.)
...
- **Manual engine build error reporting**: the Build/Pull result banner in the System view now shows the server's actual error message (e.g. "engine build already in progress") instead of the generic axios "Request failed with status code ..." text; cancel actions likewise surface API errors as toasts instead of unconditionally reporting success.

### Optimized
- **Upstream Layer Deduplication**: Consolidated repeated code in the upstream stack — the store now shares a single column list with `scanUpstream`/`queryUpstreams` helpers (removing three near-identical `SELECT`+`Scan` blocks and the column-drift risk) and a shared INSERT column set/args helper; the HTTP handler shares one embedded `upstreamConfigFields` struct with a `toUpstream()` mapper (single source for the type whitelist and request→model conversion across the add/update/test/bulk requests); the service shares a `tcpProbe` helper between the SOCKS4 and Shadowsocks testers and reuses the existing `resolveTestResult` in the manual-test and initial-health-check paths. No behavior change.
- **Go Report Card cleanup**: the repository now passes the full goreport-style suite (gofmt, go vet, gocyclo ≤ 15, staticcheck, ineffassign, misspell, golangci-lint). Seven functions above the cyclomatic-complexity threshold were decomposed into focused helpers with no behavior change — upstream `BulkCheck` (proxy-line check, SOCKS4-retry predicate, result builder), the audit list handler/store (query-param parsing, SQL `WHERE` builder with a shared `IN`-clause helper), `ConfigHandler.Update` (key whitelist filter, Middle-Proxy guard), `ParseProxyLine` (bracketed-IPv6 and colon-form parsers), and `GenerateBulkUpstreamName` (prefix/host-port/hash/sanitize helpers). Two long-standing unformatted files were brought under gofmt, and the generated Swagger docs were refreshed with the new endpoints and fields.
