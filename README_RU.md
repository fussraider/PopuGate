...
### 🔀 Upstreams
Цепочки прокси (SOCKS4/SOCKS5/Shadowsocks) с балансировкой весов, привязкой к сетевым интерфейсам и автоматическим переключением при сбоях (auto-failover/recovery).
- **Пакетное добавление**: Проверка и импорт множества прокси одновременно через пошаговый мастер с автоопределением протокола (`socks5://`, `socks4://`, `ss://`), живым запросом выходного IP, фильтрацией по пингу и пакетной транзакционной вставкой в базу данных. При включённом режиме Middle-Proxy строки `ss://` пропускаются с предупреждением, не отменяя импорт всей пачки.
- **Shadowsocks**: Выход через сервер Shadowsocks по его `ss://`-ссылке. Требует **отключённого режима Middle-Proxy** в настройках (движок отклоняет Shadowsocks-апстримы при активном Middle-Proxy / `ad_tag`; UI и API не дают включить эту комбинацию).
...
- **Интеграция с Docker**: Проверка статуса демона Docker на хосте, обновление Docker, сборка и обновление образа прокси-движка `telemt`. Обновление движка выполняется асинхронно в фоне (с защитным таймаутом) и — как и ручные Build/Pull и Force Rebuild — может быть прервано прямо во время сборки; сворачиваемая панель **Логи сборки** показывает лог текущего запуска с индикатором активности, а последняя неудачная попытка обновления отображается отдельным баннером с ошибкой.
...
Глобальные параметры: CPU/memory лимиты Docker, кастомный IP, FakeTLS, PROXY-протокол, кастомные URL Telegram, Ad Tag, переключатель режима Middle-Proxy, авторотация секретов, режим обслуживания, ротация бекапов, режим отладки.
